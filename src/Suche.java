public class Suche {
    ...
    int targetCol, targetRow; //Variablen f?r Koordinaten des Target anlegen
    ...
    public Suche (String user, int maxCol, int maxRow, int[][] map, int targetCol, int targetRow) {
        this.user = user;
        this.maxCol = maxCol;
        this.maxRow = maxRow;
        this.map[][] = map[][];
    ...
    public void getDetails () {
        ...
        map[0][0] = 1; //Markiere die Position des Hamsters mit 1 (immer oben links)
    }
    ...
    void suchePfad () {
        ...
        while (!found){
            ...
            ind++; //Erh?he den Index um 1
        }
        int row = targetRow, col = targetCol;
        route.erstellePfad(ind, row, col, map);
        route.hamsterLauf();
    }

    void mark(int col, int row, int ind){
        //Methode, die g?ltige Nachbarn makiert
